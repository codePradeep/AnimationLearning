...
const collectIceCandidtes=(cRef: FirebaseFirestoreTypes.DocumentReference<FirebaseFirestoreTypes.DocumentData>,localname: string,remoteName: string)=>{
...
const streamCleanUp= async ()=>{
  if(localStream){
    localStream.getTracks().forEach((a)=>a.stop());
    localStream.release()
  }
  setLocalStream(null)
...
//display local streaming on calling
 if(localStream){
  return(
    <VideoScreen hungup={hungup} localStream={localStream} remoteStream={remoteStream} />
  )
 }
 ...
 //display the call button 
  return (
    <View style={{flex:1,justifyContent:"center",alignItems:"center"}}>
      <CommonButton onPress={()=>create()} iconName={'video'} style={{backgroundColor:"grey"}} />
    </View>
